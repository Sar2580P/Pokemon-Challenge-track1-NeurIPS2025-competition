...
# if __name__ == "__main__":      
    ...
    dset_path=os.path.join(METAMON_CACHE_DIR, "parsed-replays", "gen9ou")
    if not os.path.exists(dset_path):
        download_gen9ou_subset_parsed_replays(filenames=[f'gen9ou-{YEAR}.zip'], 
                                              destination_folder=dset_path)
        volume.commit()
        print(f"⚠️ Dataset of gen9ou-{YEAR} has now been downloaded, pls rerun the script to use it...")
        return
    ...
    experiment.learn_opponent_modeling()
    wandb.finish()
